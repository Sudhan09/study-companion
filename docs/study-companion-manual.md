...
| **claude.ai/code cloud session** (web/sessions UI) | **Repo-scope `.claude/settings.json` declaration** — auto-installs at session start from `Sudhan09/study-companion-marketplace` (built 2026-05-07 evening per Track 4) | **Loaded ✓** via marketplace declaration | Same as Cowork: 12 in registry, 2 hidden by `disable-model-invocation` |

**Implication:** `/pause-routines` and `/resume-routines` now work on **all three surfaces** post-2026-05-07-evening marketplace setup. Cowork remains the **preferred** surface per Path A v3 Q2 — its full `gh run list` poll gives "merged into main at `<SHA>`" confirmation, vs claude.ai/code cloud session's push-only "watch Slack for merge" confirmation. Both eventually result in the same merged-pause state on main; the difference is feedback richness.
...
1. From Cowork or Code app: `/pause-routines`. **Cowork is the preferred surface per Q2** (full `gh run list` poll → "merged into main at `<SHA>`" confirmation). claude.ai/code cloud sessions also work post-2026-05-07-evening marketplace setup but only get push-only confirmation. Local Claude Code CLI (when plugin is locally installed) behaves like Cowork.
...
**On claude.ai/code cloud session (post-2026-05-07-evening marketplace setup):** Same as Cowork — case A (registry blind spot for `disable-model-invocation: true` skills). The plugin auto-installs at session start via the granted folder's `.claude/settings.json` declaring `Sudhan09/study-companion-marketplace` + `enabledPlugins: { study-companion: true }`. All 14 skills are loaded; pause/resume hidden from auto-routing but user-invokable via `/pause-routines`. **Pre-marketplace (2026-05-07 morning + earlier):** plugin was genuinely not loaded on cloud sessions because granted folder had no `.claude/settings.json`. If you see "0 plugin skills" in a cloud session, the marketplace declaration may have been removed — check `git show HEAD:.claude/settings.json`.
...
|---|---|---|
| Lists 0 plugin skills | Marketplace declaration missing or broken (was the pre-marketplace state) | Check `cat .claude/settings.json` — if absent or malformed, restore from `git show HEAD:.claude/settings.json` |
| Lists exactly 12 (calibrate, companion, day-wrap, drill, lock-decision, lock-weak-spot, post-session, rti-preflight, self-review, teach, teach-from-win, trace) | Normal — `disable-model-invocation: true` blind spot for pause/resume | Type `/pause-routines` directly; ignore the registry omission |
| Lists all 14 | Fully loaded; original "doesn't exist" was a model-introspection error | Move on |
...
│   ├── .claude/
│   │   └── settings.json          # marketplace + enabledPlugins (cloud-session loader)
│   ├── .github/
...
│   ├── README.md
│   └── SECURITY.md
├── study-companion-marketplace/   # marketplace repo (built 2026-05-07 evening, Track 4)
│   ├── .claude-plugin/
│   │   └── marketplace.json       # declares study-companion plugin sourced from study-companion-plugin
│   └── README.md
...
- **Marketplace repo:** https://github.com/Sudhan09/study-companion-marketplace (built 2026-05-07 evening; declares study-companion plugin for cloud-session auto-install)
...
| **Marketplace + cloud-session plugin load** | ⏳ | Infrastructure landed 2026-05-07 evening: `Sudhan09/study-companion-marketplace` created with `.claude-plugin/marketplace.json` declaring study-companion. Granted folder gains `.claude/settings.json` with `extraKnownMarketplaces` + `enabledPlugins: { study-companion: true }`. **User-verification pending** — Sudhan must open a fresh claude.ai/code cloud session against the granted folder and confirm all 14 skills load (12 visible in registry; pause/resume hidden by `disable-model-invocation` but invokable via `/pause-routines`). |
